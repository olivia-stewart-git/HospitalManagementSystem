...
public class PatientAppointmentsView : View
{
	readonly IEnvironment environment;
	readonly IUnitOfWorkFactory unitOfWorkFactory;
	readonly IViewService viewService;

	public PatientAppointmentsView(IEnvironment environment, IUnitOfWorkFactory unitOfWorkFactory, IViewService viewService)
	{
	...
	{
		viewBuilder
			.AddControl(new PageHeader("DOTNET Hospital Management System", "My Appointments"))
	...
	public override void OnBecomeActive()
	{
		...
		var appointments = appointmentsRepository
			.GetWhere(x => x.APT_Patient.PAT_USR_ID == environment.CurrentUser.USR_PK,
			[
		...
		else
		{
			...
			outputBox.SetState("No appointments for patient", OutputBox.OutputState.Warn);
		}
	}
